...
This gate verifies skill and tool availability. Production PLC acceptance uses KV STUDIO conversion/compile evidence.
...
powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\kvstudio\kvtool.ps1 run-interactive `
  ...
  -MnmPath '<work-root>\input\Main.mnm'
```
...
powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\kvstudio\kvtool.ps1 run-interactive `
  ...
  -VmScriptPath '<remote-work-root>\tools\kvstudio\convert_collect.ps1' `
  -TimeoutSeconds 900
```
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File <remote-work-root>\tools\kvstudio\convert_collect.ps1 `
  -ProjectPath '<project-root>\project.kpr' `
  -OutDir '<work-root>\evidence\convert_case_001' `
  -RestartKvs
```
...
Stable promotion requires a repeatable command, explicit parameters, a known output directory, and evidence that it changes the KV STUDIO project or conversion result in the intended way.
