param(
  ...
  [string]$ProjectPath,
  [string]$OutDir = (Join-Path ([IO.Path]::GetTempPath()) 'keyence-plc-programmer\mnm_export_validation'),
...
try{
  ...
  $searchRoots = @($out, (Split-Path -Parent $out)) | Where-Object { $_ } | Select-Object -Unique
  $mnmFiles = @()
