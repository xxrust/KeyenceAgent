param(
  ...
  [string]$IsolatedHomeRoot = (Join-Path ([IO.Path]::GetTempPath()) 'keyence-plc-programmer\isolated_codex_home'),
  [string]$ArtifactRoot = (Join-Path ([IO.Path]::GetTempPath()) 'keyence-plc-programmer\isolated_agent'),
  [string]$Python = ''
)
...
if (-not $Python) {
  ...
  if (-not $Python) {
    throw 'No real python.exe found. Pass -Python <python.exe>; batch shims corrupt multiline QGA scripts.'
  }
}
