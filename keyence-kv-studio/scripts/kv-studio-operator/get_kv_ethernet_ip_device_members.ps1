...
if ([string]::IsNullOrWhiteSpace($EdsRoot)) {
  if (-not [string]::IsNullOrWhiteSpace($env:KV_STUDIO_EDS_ROOT)) {
    $EdsRoot = [Environment]::ExpandEnvironmentVariables($env:KV_STUDIO_EDS_ROOT)
  } else {
    $commonData = [Environment]::GetFolderPath('CommonApplicationData')
    if ($commonData) {
      $EdsRoot = Join-Path $commonData 'KEYENCE\KVS\EIP_Eds'
    }
  }
}
if ([string]::IsNullOrWhiteSpace($EdsRoot)) {
  throw 'EdsRoot is required. Set -EdsRoot or KV_STUDIO_EDS_ROOT.'
}
