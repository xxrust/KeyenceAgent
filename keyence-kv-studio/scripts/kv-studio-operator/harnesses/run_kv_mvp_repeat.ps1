...
$runner = Resolve-KvStudioOperatorScriptPath -ScriptRoot $scriptRoot -Name 'run_kv_mvp_scaffold.ps1' -Classes @('customer_workflow')
...
if ([string]::IsNullOrWhiteSpace($OutRoot)) {
  $OutRoot = Join-Path (Join-Path ([IO.Path]::GetTempPath()) 'kv-studio-operator') 'mvp_repeat_runs'
}
