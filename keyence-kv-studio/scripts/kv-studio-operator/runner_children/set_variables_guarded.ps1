...
function Close-VariableEditorAndSave([System.Diagnostics.Process]$Process, [string]$ProjectNeedle, [string]$Stage) {
  if (-not (Get-VariableForm $Process.Id)) {
    Log "variable editor already closed before $Stage"
  } else {
    Close-VariableEditor $Process.Id
    Start-Sleep -Milliseconds 700
    if (Wait-VariableForm $Process.Id 2) { throw "KvVariableForm remained open after close request at $Stage." }
  }
  Save-Project $Process $ProjectNeedle
  Start-Sleep -Milliseconds 500
  Assert-NoKvsModal $Process.Id "after variable editor close and save at $Stage"
  Log "variable editor closed and project saved at $Stage"
}
...
try {
  ...
  if ($localRows.Count -gt 0 -and [string]::IsNullOrWhiteSpace($LocalProgramName)) {
    throw 'LocalProgramName is required when executable local variables exist.'
  }
  ...
  if ($AuditPersistence -and -not $KeepVariableEditorOpen) {
    Close-VariableEditorAndSave $process $projectNeedle 'before persistence verification'
    Save-Shot '04_after_variable_editor_close_and_save.png'
  } else {
    Log 'persistence verification skipped or variable editor kept open by parameter'
  }
  ...
  if ($AuditPersistence -and $localRows.Count -gt 0) {
    ...
    if (-not $KeepVariableEditorOpen) {
      Close-VariableEditorAndSave $process $projectNeedle 'after local persistence verification'
    }
  }

  if (-not $KeepVariableEditorOpen -and (Get-VariableForm $process.Id)) {
    Close-VariableEditorAndSave $process $projectNeedle 'final clean end state'
  }
