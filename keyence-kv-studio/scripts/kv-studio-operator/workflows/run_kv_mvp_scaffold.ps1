...
if ([string]::IsNullOrWhiteSpace($OutRoot)) {
  $OutRoot = Join-Path (Join-Path ([IO.Path]::GetTempPath()) 'kv-studio-operator') 'mvp_runs'
}
...
$planOutDir = Join-Path ([IO.Path]::GetFullPath($OutRoot)) ('_execution_plans\new_project_' + (Get-Date -Format 'yyyyMMdd_HHmmss_ffff'))
...
if ($CpuModel) { $planArgs += @('-CpuModel',$CpuModel) }
if ($KvsExe) { $planArgs += @('-KvsExe',$KvsExe) }
if ($ChecklistPath) { $planArgs += @('-ChecklistPath',$ChecklistPath) }
if ($AdminUser) { $planArgs += @('-AdminUser',$AdminUser) }
if ($AdminPassword) { $planArgs += @('-AdminPassword',$AdminPassword) }
if ($AdminCredentialPath) { $planArgs += @('-AdminCredentialPath',$AdminCredentialPath) }
