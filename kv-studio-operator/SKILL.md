...
The script is parameterized with `-DevicePath`, but the stable evidence currently covers KEYENCE SV3 only. Do not claim third-party ESI or non-SV3 EtherCAT support until a clean-project run adds that device, handles its dialogs, saves, and passes `Ctrl+F9` with copied conversion text.
