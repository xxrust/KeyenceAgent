...
All process-wide input must go through `scripts/mvp/kv_ui_guard.ps1`.
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File <skill-root>\scripts\assert_kv_mvp_ui_guard_usage.ps1 `
  -ScriptsRoot <skill-root>\scripts\mvp `
  -OutDir <work-root>\guard_static_probe
```

Success means the MVP child scripts no longer contain raw global input outside `kv_ui_guard.ps1`.
...
- Do not run KV STUDIO.
- Inspect `kv_ui_guard_usage_findings.json`.
- Move each reported input operation into `kv_ui_guard.ps1` or a wrapper that calls it.
...
| `KV_TARGET_WINDOW_NOT_FOREGROUND` | `kv_ui_guard.ps1` | KV STUDIO is foreground, but not the target window. | Stop and inspect checkpoint; do not send input. |
