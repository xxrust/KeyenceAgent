...
$mvpScriptRoot = Join-Path $scriptRoot 'runner_children'
...
function Invoke-MvpStep([string]$Name, [string]$ScriptName, [string[]]$Arguments) {
  ...
  $scriptPath = Resolve-KvStudioOperatorScriptPath -ScriptRoot $scriptRoot -Name $ScriptName -Classes @('runner_child_approved','runner_child_pending')
...
$scaffoldValidator = Resolve-KvStudioOperatorScriptPath -ScriptRoot $scriptRoot -Name 'validate_kv_mvp_scaffold.ps1' -Classes @('customer_scaffold_tool')
...
$uiGuardUsageCheck = Resolve-KvStudioOperatorScriptPath -ScriptRoot $scriptRoot -Name 'assert_kv_mvp_ui_guard_usage.ps1' -Classes @('gate')
...
$agentBoundaryCheck = Resolve-KvStudioOperatorScriptPath -ScriptRoot $scriptRoot -Name 'assert_kv_mvp_agent_boundary.ps1' -Classes @('gate')
...
$mnmImportPlanGate = Resolve-KvStudioOperatorScriptPath -ScriptRoot $scriptRoot -Name 'assert_kv_mnm_import_plan.ps1' -Classes @('customer_scaffold_tool')
